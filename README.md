...
下载本项目到本地后，您需要在该项目的 `.\uniapp\EchoNoteCode` 目录下运行指令：

```bash
npm run dev:h5
```

即可编译运行该程序，然后在网页端打开程序。

>方法2

我们更建议您利用微信开发者工具这一开发环境运行本项目，这样您可以看到本项目在微信小程序上运行的真实画面！

首先，您需要在该项目的 `.\uniapp\EchoNoteCode` 目录下运行指令：

```bash
npm run dev:mp-weixin
```

然后在微信开发者助手上导入文件夹：`.\uniapp\EchoNoteCode\dist\build\mp-weixin` ，便可以运行该程序，界面如下：

![](./assets/%E5%BC%80%E5%8F%91%E7%8E%AF%E5%A2%83.png)

便可以在左边的环境中执行模拟点击，体验本项目的功能！:happy:
